...
#define __TIMER_DETAILS           0x0003                  //  2
#define __TIMER_INTERRUPTS        0x0003                  //  3
#define __TIM1_PERIOD             0x30D40                 //  4
...
#define __TIM2_PERIOD             0x30D40                 // 20
...
__inline static void stm32_TimerSetup (void) {
  ...
  if (__TIMER_USED & 0x01) {                                // TIM1 used
    ...
    if (__TIMER_INTERRUPTS & 0x01) {                        // interrupts used
      ...
      NVIC->ISER[0] |= (1 << (TIM1_UP_TIM16_IRQn & 0x1F));  // enable interrupt
    }
  ...
  if (__TIMER_USED & 0x04) {                                // TIM3 used
    ...
    if (__TIMER_INTERRUPTS & 0x04) {                        // interrupts used
      ...
      NVIC->ISER[0] |= (1 << (TIM3_IRQn & 0x1F));     // enable interrupt
    }
